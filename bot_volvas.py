...
def start(message):
    ...
    try:
        with open(r"users.txt", mode="w", encoding="UTF-8") as file:
...
def reply(message):
    ...
    if message.text == "Орел или решка":
        ...
        if users[message.chat.id]['count'] == 21:
            ...
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
            bot.send_message(message.chat.id, f'К сожелению вы проиграли, у бота было {users[message.chat.id]["botcount"]} ')
            repeat = types.KeyboardButton('Ну давай')
            markup.add(repeat)
            bot.send_message(message.chat.id, 'хочешь еще раз?', reply_markup=markup)
